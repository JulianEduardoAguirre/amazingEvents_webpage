let asyncIndex = async function(){
	...
	try {
		...
		// CARDS SECTION
		div_tarjetas.innerHTML = generateCards2(filterByDate2(apiEvents), allCategoriesArray);				//First time rendering cards (using all data)

		// CHECKBOXES
		// div_checkboxes.innerHTML = checkBoxGenerator(filterByDate(apiEvents));										//Use this for only checkboxes with available categories
		div_checkboxes.innerHTML = checkBoxGenerator2(allCategoriesArray);												//First (and unique) time rendering checkboxes
